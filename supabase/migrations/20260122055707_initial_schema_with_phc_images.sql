/*
  # Initial Database Schema for KWPHCDA with PHC Images

  1. New Tables
    - `users` - User profiles with role-based access control
    - `phcs` - Primary Health Care facilities (with image support)
  ...
  3. Features
    ...
    - PHC facility management with image support
    - Blog content management
    - Role-based permissions
*/

-- Create PHCs table first (users table references it)
...
CREATE POLICY "Admins can read all PHCs"
  ON phcs
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY "Admins can insert PHCs"
  ON phcs
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY "Admins can update PHCs"
  ON phcs
  FOR UPDATE
  TO authenticated
  USING (
...
CREATE POLICY "Admins can delete PHCs"
  ON phcs
  FOR DELETE
  TO authenticated
  USING (
